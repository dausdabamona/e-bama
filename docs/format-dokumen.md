...
| 10 | Rencana Pengajuan SPM per Suplier | REKAP_BULANAN + TARUNA + `TARUNA_REKENING` (rekening PENUH + `penyedia_id`, di-audit). **Dipecah per ID suplier** (tiap suplier = 1 lembar SPM), di dalamnya dikelompokkan **prodi + tingkat**. Bayar LS ke rekening taruna. **TTD PPK saja** | Cetak `/cetak/form-10` | ✅ diimplementasi (ADMIN/PPK) |
