...
| `cetak.pendebetan_penyedia` | SENAT, PPK, STAF_PPK, ADMIN | `{bulan?}` → `{bulan_filter, baris:[{nit,nama,prodi,tingkat,bulan,nominal}], total_nominal, rekening_senat:{BNI,BSI}, rekening_senat_nama, rekening_penyedia:{BNI,BSI}, rekening_penyedia_nama, pejabat}` — Surat Permohonan Pendebetan Rekening **SENAT → PENYEDIA** untuk dana tagih-ulang GAGAL DEBET yang sudah **LUNAS** & **belum diteruskan** (`tgl_diteruskan_penyedia` kosong). Sumber: `_tagihanJoin_` status `LUNAS`; `nominal` = `nilai_transfer` bila ada, jika tidak `nominal`. **TIDAK memuat rekening taruna** (debit antar rekening INSTANSI) → bukan data sensitif §4, boleh di-cache, tanpa audit khusus. Rekening Senat/Penyedia dari `getRekeningInstansi()`. Frontend `/cetak/pendebetan-penyedia`: pemilih Bank (BNI/BSI), **dibagi per bulan** (satu surat per bulan), TTD Ketua Senat + PPK + Mengetahui Direktur. Ditaut dari halaman "Teruskan ke Penyedia" | ✅ diimplementasi |
