...
| `tagihan.list` | semua login | sertakan `level_aktif` (MAX level SP) + `tenggat_aktif` + `verif_1_oleh`/`verif_2_oleh`/`nilai_transfer`/`bukti_setor_drive_file_id`/`selisih_transfer` (=nominal-nilai_transfer, dihitung) + `kebijakan:{toleransiSelisihTransfer}` (dari `getKebijakanTagihan`, `00_config.gs`); cache 60 detik, invalidate saat tulis |
