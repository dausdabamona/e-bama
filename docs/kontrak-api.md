...
| `spm.list` | PPK, KPA, SENAT, WADIR3, ADMIN | `{bulan?, bayar_id?, kategori?}` → `{spm:[{spm_id,bayar_id,kategori,bulan,prodi,tingkat,penyedia_id,penyedia_nama,kegiatan,pembayaran_ke,periode,nominal,no_spm,tgl_spm,no_sp2d,tgl_sp2d,status}]}` — daftar baris `SPM` tersimpan (bukan perkiraan). `penyedia_nama` di-join dari PENYEDIA (kosong bila tak ketemu). Frontend `pembayaran.tsx`: bulan dgn `spm.length>0` → tampilkan kartu "SPM Dalam Kampus" (`spm-dalam-kampus.tsx`, bisa diedit inline), GANTI kartu "Rincian SP2D" legacy |
...
| `spm.regenerate` | PPK, ADMIN | `{bayar_id}` → hapus & buat ulang SPM `DALAM_KAMPUS` dari REKAP_BULANAN terbaru; HANYA bila SEMUA baris grup itu masih `DRAFT` (dipakai kalau REKAP dikoreksi setelah SPM dibuat, sebelum diajukan) |
...
| `tagihan.summary` | PPK, KPA, WADIR3 | `{per_level: {0..3: {jumlah, nominal}}, total_outstanding, sudah_disetor_menunggu_verifikasi_1: {jumlah,nominal}, verifikasi_1x: {jumlah,nominal}, lunas_belum_diteruskan: {jumlah,nominal}, lunas_sudah_diteruskan: {jumlah,nominal}}` — dashboard piutang. `sudah_disetor_menunggu_verifikasi_1` = tagihan TERTAGIH yang `tgl_setor`-nya SUDAH terisi (dana sudah masuk ke rekening Senat) tapi BELUM punya verifikator sama sekali — tahap PALING AWAL (dikonfirmasi Firdaus, sebelumnya angka ini tak muncul di dashboard, cuma kelihatan di sub-grup "Menunggu Verifikasi ke-1" per bulan). `verifikasi_1x` = tagihan TERTAGIH yang sudah punya verifikator pertama (menunggu verifikator kedua) — TAHAP SETELAH `sudah_disetor_menunggu_verifikasi_1`, dua bucket ini saling eksklusif. `lunas_belum_diteruskan`/`lunas_sudah_diteruskan` pakai `nilai_transfer` (fallback `nominal`) dan dipisah berdasar `tgl_diteruskan_penyedia` — `lunas_belum_diteruskan` adalah angka "dana tagih-ulang yang sudah dikumpulkan tapi belum diteruskan ke penyedia" (TERPISAH dari status SP2D/SPM jalur pembayaran LS utama) |
