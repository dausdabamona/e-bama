...
| Action | Role | Keterangan |
|---|---|---|
| `status.set` | ADMIN, PEMBINA, BAAK, PPK, STAF_PPK | `{tanggal, nit, status, berkas?, tgl_akhir?}` — upsert per (tanggal, nit); BAAK dipakai untuk surat taruna keluar kampus (PKL) & surat penarikan kembali, lampirkan lewat `berkas`. **`tgl_akhir` opsional**: bila diisi, membuat satu baris STATUS_HARIAN per hari dari `tanggal` s.d. `tgl_akhir` inklusif (maks 186 hari) — utk status berdurasi (cuti/sakit/PKL berhari-hari) tanpa input per hari. **Khusus `TANPA_KETERANGAN` tanpa `tgl_akhir`** (waktu kembali tak diketahui): otomatis diisi maju s.d. **akhir kontrak katering aktif** (fallback akhir bulan berjalan, clamp 185 hari) — "berlaku sampai dicabut"; dicabut saat taruna kembali via `status.tandai_kembali`. Status lain tanpa `tgl_akhir` tetap 1 hari. Balikan: `{status_id, aksi}` bila 1 hari, `{jml}` bila rentang |
| `status.batch` | ADMIN, PEMBINA, BAAK, PPK, STAF_PPK | `{tanggal, status, nit:[], berkas?, tgl_akhir?}` — input massal (mis. satu kelas pesiar); `tgl_akhir` idem `status.set` (per taruna × per hari dalam rentang), termasuk perilaku `TANPA_KETERANGAN` open-ended s.d. akhir kontrak. Balikan `{jml}` = jumlah baris (taruna×hari) |
...
| `kajur.set_tarif` | KETUA_JURUSAN | `{bulan, kegiatan, nilai_per_hari, nit_list:[], pembayaran_ke?}` → `{jml, nilai_per_hari, kegiatan}` — **isi harga satuan (tarif/hari) di aplikasi tanpa impor CSV** (Ketua Jurusan tahu harga daerah setempat). Upsert baris BANTUAN_LUAR_KAMPUS per (nit, kegiatan, bulan, pembayaran_ke) untuk taruna prodinya; `total_hari`/`nominal` di-snapshot dari STATUS_HARIAN saat ini (rekap tetap hitung ulang hari×tarif). Mengubah tarif → status baris kembali `DRAFT` (perlu approve ulang). Scope prodi (`_hanyaKajur_`), withLock + audit per baris |
| `kajur.tanggal_taruna` | KETUA_JURUSAN | `{nit, bulan}` → `{nit, bulan, tanggal:[{tanggal,status}], hari, min, max}` — daftar TANGGAL luar kampus satu taruna pada bulan (untuk editor kalender & ringkasan periode). READ murni, scope prodi |
| `kajur.hapus_absen` | KETUA_JURUSAN | `{nit, tanggal?}` \| `{nit, dari, sampai}` \| `{nit, bulan}` → `{jml_dihapus, nit}` — **KOREKSI** hapus absen luar kampus satu taruna. `tanggal` → satu hari (kalender); `dari`+`sampai` → satu RENTANG (lintas bulan boleh — mis. berhenti KPA lebih awal, potong 1–21 Mei); `bulan` → semua hari bulan itu. BEDA dari `status.tandai_kembali` (hanya hari KE DEPAN) — boleh tanggal LAMPAU. HANYA status luar kampus (tak mengubah Pesiar/Cuti/Sakit). Scope prodi, withLock + audit (catat tanggal/status yg dihapus) |
| `periode.impor` | ADMIN | `{baris:[{nit,status,tgl_mulai,tgl_akhir}]}` → `{dibuat, dobel, dilewati_nit:[…]}` — **impor massal** periode luar kampus (mis. rekap PKL/KPA se-angkatan dari Excel/CSV). Validasi format all-or-nothing; NIT tak ada di TARUNA DILEWATI & dilaporkan (tak menggagalkan); duplikat (nit+status+tgl_mulai+tgl_akhir) dilewati → aman diulang. Frontend: kartu "📥 Impor Periode Luar Kampus dari CSV" (ADMIN) di /status-taruna |
| `luar.migrasi_periode` | ADMIN | `{dry_run?}` → dry: `{dry_run, luar_rows, periode, sisa_status_harian, contoh:[…]}`; real: `{migrasi, periode, sisa_status_harian}` — **MIGRASI sekali jalan** (Tahap 5): kolaps baris STATUS_HARIAN luar kampus (per hari) → PERIODE_LUAR (rentang berurutan per nit+status; jeda memecah periode → hari efektif TETAP). Menghapus baris legacy dgn **tulis-ulang sheet** (bukan deleteRow per baris → hindari timeout ribuan baris). `dry_run` = pratinjau tanpa ubah. Aman diulang (pembacaan hari dedup per-tanggal). Frontend: kartu "🛠️ Migrasi Luar Kampus → Periode" (ADMIN) di /status-taruna |
| `kajur.periode_set` | KETUA_JURUSAN | `{nit_list, status, tgl_mulai, tgl_akhir}` (CREATE) \| `{periode_id, status, tgl_mulai, tgl_akhir}` (UPDATE/potong) → `{jml, ...}` — **model periode**: 1 baris PERIODE_LUAR per taruna (hemat, bukan ribuan baris/hari). `status` ∈ STATUS_LUAR_KAMPUS. Scope prodi, withLock + audit |
| `kajur.periode_list` | KETUA_JURUSAN | `{}` → `{prodi, periode:[{periode_id,nit,nama,tingkat,status,tgl_mulai,tgl_akhir}]}` — daftar periode luar kampus prodinya (READ) |
| `kajur.periode_hapus` | KETUA_JURUSAN | `{periode_id}` → `{jml_dihapus}` — hapus satu periode (scope prodi, withLock + audit) |
...
| Action | Role | Keterangan |
|---|---|---|
| `realisasi.create` | PEMBINA, SENAT, PPK, STAF_PPK | pesanan wajib TERKIRIM; porsi, ketidaksesuaian, geotag; foto via lampiran `jenis=FOTO` (`berkas` close-up + `berkas_wide` wide-shot). Tahap 2 foto masukan taruna: `berkas_tambahan` = array `{base64, nama_file}` foto sudut tambahan OPSIONAL (maks 4, disimpan `jenis=FOTO`) + `berkas_pdf` = `{base64, nama_file}` lampiran dokumen OPSIONAL (mis. berita acara serah-terima penyedia, disimpan `jenis=LAINNYA`, mime otomatis dari `.pdf`). Ownership Taruna Fitur 1b: field `piket_nit`/`piket_menu_sesuai`/`piket_porsi_cukup`/`piket_kualitas`/`piket_gizi[]`/`piket_catatan` OPSIONAL (wajib bila `getKebijakanPiket().wajib`, default false) — NIT divalidasi ke roster TARUNA, `piket_kualitas` wajib salah satu `BAIK`/`CUKUP`/`KURANG` begitu `piket_nit` diisi, `piket_gizi` divalidasi terhadap `getKebijakanGizi().komponen`. MENAMBAH bukti, TIDAK mengubah ttd/foto/geotag yang sudah ada |
...
| `realisasi.ttd_massal` | PEMBINA, SENAT | `{real_ids:[], pin}` → `{ditandatangani, lengkap, dilewati, bulan:[]}` — tanda tangan BANYAK hari sekaligus, untuk mengejar bulan tertunda tanpa membuka puluhan halaman. Hanya mengisi kolom ttd **role pemanggil**; baris yang sudah ia tandatangani **DILEWATI** (bukan menggagalkan batch → aman diklik ulang/idempotent); `lengkap` = jumlah hari yang menjadi ber-ttd LENGKAP karena batch ini. `rekapUpdate` dipanggil **sekali per bulan** yang tersentuh di akhir (bukan per baris — satu panggilan sudah menghitung ulang sebulan penuh). **Tiga pagar kontrol** karena tanda tangan adalah bukti pertanggungjawaban: (1) `real_ids` WAJIB eksplisit — tidak ada mode "semua bulan ini"; (2) kata sandi tetap wajib (diverifikasi sekali per batch, helper `_konfirmasiKataSandi_`); (3) **satu baris AUDIT_LOG per realisasi** atas nama penandatangan, bukan satu baris untuk seluruh batch. Maks `_TTD_MASSAL_MAKS_` (40) per panggilan demi kuota eksekusi GAS; `real_ids` kosong / id tak dikenal → batal SEBELUM ada baris tersentuh. Dibungkus `withLock` |
...
| `realisasi.penerimaan` | SENAT, PEMBINA, ADMIN | `{real_id? , pesanan_id?, penerimaan:{pagi,siang,malam:[{komponen,ada,jumlah,keterangan?,sesuai?}]}}` — checklist Penerimaan Barang Senat per waktu makan × komponen. `sesuai` (Tahap 3, kesesuaian menu kontrak) = boolean, DEFAULT true, hanya `false` bila ditandai tidak sesuai (backward-compatible; frontend mewajibkan `keterangan` saat `sesuai=false`). (`komponen` = nama menu bebas non-kosong maks 40 karakter — TIDAK lagi dikunci ke `getKebijakanKomponenMenu().komponen` supaya menu non-standar per hari mis. "Bubur Kacang Ijo"/lauk kedua/"Milo" bisa direkam apa adanya [masukan taruna]; daftar kebijakan tetap dipakai frontend sebagai checklist standar & di `realisasi.rekap_penerimaan` [komponen di luar daftar dilewati di rekap, tetap tersimpan utuh]. `jumlah` integer ≥ 0, kunci waktu selain pagi/siang/malam ditolak). `keterangan` OPSIONAL, bebas isi maks 60 karakter — dipakai mis. mencatat jenis lauk/minuman nyata (Ikan/Ayam/Teh/Susu dst), TIDAK dikunci enum. Komponen standar (`getKebijakanKomponenMenu`) kini: Nasi, Sayur, Lauk, Buah, Minuman, Kerupuk. Realisasi TERKAIT harus SUDAH ADA (dicari via `real_id`, atau via `pesanan_id` bila `real_id` tak diberikan) — kalau belum ada, tolak dengan pesan "buat realisasi.create dulu". TERPISAH dari `realisasi.ttd` (decouple, boleh diisi kapan saja). MENAMBAH data, TIDAK mengubah `porsi_diterima`/`jml_taruna_makan`/ttd/`piket_*` yang sudah ada |
...
| `realisasi.rekap_penerimaan` | PPK, KPA, WADIR3, SENAT (baca) | `{bulan, penyedia_id?}` → `{bulan, total_realisasi, per_komponen:[{komponen,kali_ada,kali_tidak_ada,persen_lengkap,total_jumlah,total_selisih,kali_tidak_sesuai}], persen_lengkap_keseluruhan, komponen_paling_sering_kurang, total_tidak_sesuai_kontrak, komponen_paling_sering_tidak_sesuai}` (`kali_tidak_sesuai`/`total_tidak_sesuai_kontrak` = Tahap 3, hitung komponen ber-`sesuai=false` — bahan evaluasi kepatuhan menu penyedia) — turunan MURNI baca dari `REALISASI.penerimaan` (parse JSON), TIDAK menulis apa pun. `total_selisih` = SUM(`porsi_diterima` − jumlah tercatat) hanya saat komponen dicentang ada (indikasi kurang kuantitas). `penyedia_id` menyaring lewat `PESANAN.kontrak_id → KONTRAK.penyedia_id`. Bahan evaluasi penyedia |

### Rekap Bulanan (SOP no. 10)

| Action | Role | Keterangan |
|---|---|---|
| `rekap.get` | PPK, KPA, WADIR3 | `{bulan}` → `{rekap, total, bulan, D, total_hari_makan, basis_pesanan, ambang_outlier}` — **`basis_pesanan`** = pembanding DUA DASAR (permintaan Firdaus): `{bulan, hari_dipesan, oh_dipesan, nominal_proyeksi}` dihitung **on-the-fly** dari PESANAN status final (DISETUJUI/TERKIRIM) bulan itu — `oh_dipesan` = Σ `jml_taruna` (orang-hari, sebanding Σ `hari_makan`), `nominal_proyeksi` = Σ `jml_taruna × tarif kontrak baris pesanan` (`_hargaPerHariKontrak_`); TIDAK PERNAH ditulis ke REKAP_BULANAN (snapshot §5 tetap basis realisasi sah sebagai dasar bayar). `total_hari_makan` = Σ `hari_makan` seluruh baris (sisi realisasi utk perbandingan). Nominal ikut terkirim ke SENAT/PEMBINA mengikuti pola lama action ini — halaman `/rekap-ringkas` yang menyembunyikan. `rekap` baris mentah REKAP_BULANAN (nit, hari_makan, hari_tidak_makan, nominal, status, dst; nama/prodi/tingkat di-join FRONTEND dari `taruna.list` yang sudah ter-cache offline, TIDAK diduplikasi di sini); `D` = hari realisasi sah bulan itu (`hari_makan + hari_tidak_makan` baris pertama — konstan utk semua taruna AKTIF); `ambang_outlier` dari `getKebijakanRekap()` (Script Property `KEBIJAKAN_REKAP`, default 3) — dipakai frontend utk penanda anomali (Redesign Rekap Bulanan), MURNI tampilan, tidak memengaruhi hitungan nominal |
...
| `rekap.recompute` | PPK, STAF_PPK | `{bulan}` → `{bulan, hari_sah, taruna}` — **pemicu MANUAL** `rekapUpdate()` supaya PPK/Staf PPK bisa MEMANTAU & membentuk rekap **bulan berjalan** tanpa menunggu bulan tutup (sebelumnya rekap hanya terbentuk otomatis saat `realisasi.ttd` melengkapi kedua ttd, sehingga bulan tanpa ttd tampak kosong tanpa jalan keluar — mis. penyiapan kuasa debet taruna wisuda di tengah bulan). Rumus TIDAK berubah: yang dihitung tetap HANYA hari realisasi SAH (kedua ttd) — bila belum ada realisasi ter-ttd hasilnya memang 0 (alat pantau, bukan pengganti tanda tangan). Bulan berjalan dihitung s.d. **hari ini**, bulan lampau s.d. akhir bulan; **bulan masa depan ditolak**; bulan `FINAL` ditolak (guard di `rekapUpdate`). Aksi tulis: `withLock` + 1 baris AUDIT_LOG (`rekap.update`, kini mencatat session pemanggil) |
...
| `cetak.blokir_gagal_debet` | ADMIN, PPK, STAF_PPK | `{bulan?}` → `{bulan_filter, baris:[{nit,nama,prodi,tingkat,bulan,sebab,bank,no_rekening_lengkap,nama_pemilik,nominal,nilai_debet,rekening_lengkap_ada}], total_nominal, biaya_admin_bank, pejabat, rekening_senat:{BNI,BSI}, rekening_senat_nama:{BNI,BSI}}` — Surat Permohonan Pemblokiran & Pendebetan ke bank untuk taruna GAGAL DEBET yang BELUM MENYETOR ke Senat (jalur TAGIHAN, TERPISAH dari Form-07/SP2D). Sumber: TAGIHAN status `TERTAGIH` dengan `tgl_setor` kosong (= bucket `belum_disetor` di `tagihan.summary`); `bulan` kosong = semua tunggakan belum-setor, diisi = bulan itu. `nilai_debet` = nominal tunggakan **DIKURANGI biaya admin bank per rekening** (`getKebijakanPendebetan`, default Rp10.000 — sama kebijakan Form-07/09); `nominal` tetap utang penuh; `total_nominal` = SUM nilai_debet. Menampilkan **nomor rekening PENUH** → `_hanyaAdminPPK_` (ADMIN/PPK/STAF_PPK), **WAJIB 1 baris AUDIT_LOG** per panggilan (catat NIT terbaca, TANPA nomor). Frontend `/cetak/blokir-gagal-debet`: dikelompokkan per bank (BSI & BNI **surat terpisah**), TIDAK di-cache Dexie | ✅ diimplementasi |
| `cetak.pendebetan_penyedia` | SENAT, PPK, STAF_PPK, ADMIN | `{bulan?}` → `{bulan_filter, baris:[{tagihan_id,nit,nama,prodi,tingkat,bulan,nominal}], total_nominal, rekening_senat:{BNI,BSI}, rekening_senat_nama, rekening_penyedia:{BNI,BSI}, rekening_penyedia_nama, pejabat}` — Surat Permohonan Pendebetan Rekening **SENAT → PENYEDIA** untuk dana tagih-ulang GAGAL DEBET yang sudah **LUNAS** & **belum diteruskan** (`tgl_diteruskan_penyedia` kosong). Sumber: `_tagihanJoin_` status `LUNAS`; `nominal` = `nilai_transfer` bila ada, jika tidak `nominal`. `tagihan_id` disertakan agar halaman bisa menandai "sudah diteruskan" lewat `tagihan.teruskan_penyedia` langsung dari surat. **TIDAK memuat rekening taruna** (debit antar rekening INSTANSI) → bukan data sensitif §4, boleh di-cache, tanpa audit khusus. Rekening Senat/Penyedia dari `getRekeningInstansi()`. Frontend `/cetak/pendebetan-penyedia`: pemilih Bank (BNI/BSI), **dibagi per bulan** (satu surat per bulan), TTD Ketua Senat + PPK + Mengetahui Direktur; di bawah tiap surat ada panel **"Konfirmasi Bank Sudah Proses"** (centang taruna default semua + unggah notifikasi bank → `tagihan.teruskan_penyedia`, mendukung konfirmasi sebagian) sehingga taruna yang selesai langsung hilang dari daftar tagihan aktif & dari surat. Ditaut dari halaman "Teruskan ke Penyedia" | ✅ diimplementasi |
| `cetak.surat_pendebetan_bank` | SENAT, PPK, STAF_PPK, ADMIN | `{bulan}` (WAJIB) → `{bulan, total_nominal, jml_taruna, rekening_senat:{BNI,BSI}, rekening_senat_nama, rekening_penyedia:{BNI,BSI}, rekening_penyedia_nama, pejabat}` — Surat Pendebetan ke **BANK** (dokumen ke-1 dari pemisahan tagih-ulang): hanya **nominal TOTAL** per bulan, **TANPA nama taruna** (bank cukup: debit rekening Senat → rekening penyedia sejumlah Rp X). Sumber SAMA PERSIS dgn `cetak.laporan_penyaluran_penyedia`: `_tagihanJoin_` status `LUNAS` && `!tgl_diteruskan_penyedia` && `bulan == {bulan}`; nilai = `nilai_transfer` bila ada, jika tidak `nominal` → `total_nominal` HARUS = laporan penyaluran & = `tagihan.summary.lunas_belum_diteruskan` bulan itu. `bulan` WAJIB (tidak boleh gabung lintas bulan). **Rekening penyedia dari KONTRAK aktif bulan itu (`rek_penyedia_bni/bsi`), fallback Script Property** `getRekeningInstansi()` — sama pola Form-07/09 (kontrak dicari `_kontrakAktifPada_(bulan+'-15')`). Rekening Senat + nama a.n. dari Script Property. Data non-sensitif (debit antar rekening INSTANSI, tanpa rekening/nama taruna) → boleh di-cache, tanpa audit khusus. Frontend: tombol "Surat ke Bank" per bulan | ✅ diimplementasi (Tahap 1) |
| `cetak.laporan_penyaluran_penyedia` | SENAT, PPK, STAF_PPK, ADMIN | `{bulan}` (WAJIB) → `{bulan, baris:[{nama, prodi, tingkat, tgl_setor, jumlah}], total_nominal, penyedia_nama, pejabat}` — Laporan Penyaluran ke **PENYEDIA** (dokumen ke-2 pemisahan tagih-ulang): RINCIAN nama taruna yang sudah menyetor + jumlah + tgl setor, sebagai pertanggungjawaban ke penyedia. **TANPA nomor rekening taruna** (cukup nama + prodi/tingkat + tgl + jumlah). Filter SAMA PERSIS dgn `cetak.surat_pendebetan_bank` → `total_nominal` **HARUS identik** dgn surat bank & = `tagihan.summary.lunas_belum_diteruskan` bulan itu. `jumlah` = `nilai_transfer` bila ada, jika tidak `nominal`. Diurutkan prodi → tingkat → nama. `bulan` WAJIB. `penyedia_nama` dari `getRekeningInstansi().penyedia_nama` (BNI, fallback BSI). Non-sensitif (tanpa rekening taruna) → boleh di-cache. Frontend: tombol "Laporan ke Penyedia" per bulan | ✅ diimplementasi (Tahap 2) |
| `cetak.rekap_pesanan_bulan` | SENAT, PEMBINA, PPK, STAF_PPK, ADMIN | `{bulan}` (WAJIB) → `{bulan, baris:[{pesanan_id,tgl_makan,hari,jml_taruna,status,menu,catatan,created_by}], tanggal_kosong:[YYYY-MM-DD,...], hari_kalender, basis:{hari_dipesan,oh_dipesan,nominal_proyeksi}}` — Rekap PESANAN satu bulan penuh (laporan kontrol tambahan, **BUKAN** salah satu dari 10 form SOP). `baris` memuat **SEMUA status** (termasuk DRAFT/DIAJUKAN/DIKEMBALIKAN — supaya alur verifikasi yang macet ikut kelihatan), diurutkan tanggal. **`tanggal_kosong`** = tanggal kalender bulan itu yang SAMA SEKALI tidak punya baris PESANAN apa pun — deteksi celah seperti Juli 2026 (lihat riwayat perbaikan `pesananOtomatis21`, `20_trigger.gs`) sebelum jadi masalah di rekap bulanan. `basis` = `_basisPesananBulan_(bulan)` (14_rekap.gs, SATU rumus dgn `rekap.get`/`penyedia.portal`) — hanya menghitung pesanan status final (DISETUJUI/TERKIRIM). Read-only, tanpa audit (tak ada data sensitif). Frontend `/cetak/rekap-pesanan-bulan`, ditautkan dari "Alat Bantu Cetak" di indeks Cetak | ✅ diimplementasi |
...
### Dashboard Rekap Bulan Berjalan (`dashboard.running`)

> Modul `backend/src/27_dashboard.gs`. READ-ONLY, tanpa efek samping — agregasi
> murni dari PESANAN/REALISASI/REKAP_BULANAN/KONTRAK/TARUNA bulan itu, TIDAK
> menulis apa pun. Menggabungkan fungsi yang sudah ada (`_basisPesananBulan_`,
> `_hargaPerHariKontrak_`, `_tarunaAktifBulan_`, `_hariDalamMinggu_`) — hanya
> breakdown per-tanggal (`daily[]`) yang logika baru (tidak ada fungsi existing
> yang menghasilkan itu). PENYEDIA ikut diizinkan (dikonfirmasi Firdaus) — HARUS
> juga terdaftar di allowlist `PENYEDIA_ACTIONS` (pola sama `penyedia.portal`).

| Action | Role | Payload → Data | Keterangan |
|---|---|---|---|
| `dashboard.running` | PPK, STAF_PPK, PENYEDIA | `{bulan:1-12, tahun:YYYY}` → `{bulan, tahun, bulan_str, rekap_status, summary:{total_hari_kalender,hari_sudah_dipesan,hari_belum_dipesan,hari_sudah_direalisasi,hari_belum_direalisasi,total_porsi_dipesan,total_porsi_direalisasi,jml_taruna_aktif}, progress:{persen_pesanan,persen_realisasi}, daily:[{tanggal,hari,pesanan_id,status_pesanan,jml_taruna_pesanan,status_realisasi,porsi_diterima,jml_taruna_makan,nominal_hari}], estimation:{basis,nominal_proyeksi_pesanan,nominal_realisasi_sah,estimasi_tagihan_bulan_ini,selisih}}` — "hari sudah dipesan" = ADA baris PESANAN tanggal itu **apa pun statusnya** (beda sengaja dari `_basisPesananBulan_` yang hanya DISETUJUI/TERKIRIM: di sini progres KERJA Senat, bukan yang sudah lolos verifikasi Pembina). Progress % terhadap TOTAL HARI KALENDER (bukan terhadap hari yang sudah dipesan). `estimasi_tagihan_bulan_ini` = nilai REKAP_BULANAN bila bulan itu sudah ada barisnya, fallback `_basisPesananBulan_` — BUKAN sheet TAGIHAN (piutang gagal debet, konsep berbeda). Frontend `/dashboard-running` (PPK/STAF_PPK) — Summary Card, Progress Card (bar %), Statistik, Tabel Harian (sortir kolom + cari), rincian Estimasi Tagihan | ✅ diimplementasi |
...
| `laporan.resmi` | PPK, KPA, WADIR3, ADMIN | data untuk format "Laporan Bulanan Pemantauan & Evaluasi Bantuan Biaya Makan" resmi (acuan Itjen/KKP) — HANYA bagian Dalam Kampus yang dilacak e-BAMA (info umum, data penerima, realisasi, penggunaan dana, sebagian permasalahan); bagian rencana anggaran/Pengusulan diisi manual di halaman cetak. `kontrak` sub-objek += `harga_per_hari_efektif` (dipakai "Rencana Biaya per Orang per Hari"/"Standar Biaya per Hari"/"Besaran pendebetan per hari"). **Balikan kini juga OTOMATIS**: `sp2d_dalam_kampus[]`/`sp2d_dalam_total` (rincian SP2D Dalam Kampus dari SP2D_MONITORING agregat → B.2), `sp2d_luar_kampus[]`/`sp2d_luar_total`, `luar_kampus[]`/`luar_kampus_total`/`luar_kampus_orang` (rincian Luar Kampus dari BANTUAN_LUAR_KAMPUS per Kegiatan+Prodi+Tingkat → B.3), dan `konstanta` (DIPA/Pagu/SBM/No.SK tersimpan, prefill Bagian III) |
| `laporan.konstanta_set` | PPK, STAF_PPK, KPA, ADMIN | `{dipa_no?, pagu_tahun?, sbm?, sk_kaban?, sk_kpa?, sk_ppk?}` → simpan konstanta perencanaan Laporan Resmi (Bagian III) yang relatif tetap tiap bulan, ke Script Property `LAPORAN_KONSTAN` (merge per-kunci, pola `getRekeningInstansi`). Auto-prefill di `laporan.resmi.konstanta`. "Alokasi bulan ini" TIDAK disimpan (beda tiap bulan — manual + saran hitung di UI). Aksi tulis → withLock + AUDIT_LOG |
...
| `cetak.kuasa_debet_keluar` | **ADMIN, PPK, STAF_PPK** | `{bulan, nit_list[], basis?, sampai_tanggal?}` → shape SAMA `cetak.form07` TAPI `pembayaran` bisa `null`, + `basis` (`'REKAP'`\|`'PESANAN'`) dan `sampai_tanggal` (echo apa yang dipakai). Dokumen **kuasa debet taruna KELUAR KAMPUS** (wisuda/magang/pindah) untuk SUBSET terpilih (nit_list wajib) — nominal bulan berjalan, `nilai_debet = nominal − biayaAdminBank`, per bank, tiap taruna TTD (kuasa debet). Reuse `_daftarKuasaDebet_(bulan, nit_list, basis, sampaiTanggal)` (dipakai bersama `cetak.form07`, yang TIDAK mengirim `basis`/`sampai_tanggal` → tetap REKAP_BULANAN bulan penuh). **`basis` default `'REKAP'`** (realisasi sah, REKAP_BULANAN); **`'PESANAN'`** = proyeksi dari `_rekapProyeksiPesanan_` (14_rekap.gs) — dipakai wisuda sebelum rekap bulan berjalan terbentuk. **`sampai_tanggal`** (opsional, 'YYYY-MM-DD', HANYA berlaku basis PESANAN, harus di dalam `bulan`) — batasi proyeksi tanggal 1 s.d. tanggal ini (dikonfirmasi Firdaus: taruna wisuda di tengah bulan tidak boleh ikut menghitung hari SETELAH tanggal keluarnya); kosong = seluruh bulan. **PEMBAYARAN OPSIONAL** (dokumen bisa disiapkan SEBELUM pembayaran dibuat — beda dari form07 yang mewajibkannya): bila ada → header SPM/kontrak terisi, bila belum → `pembayaran:null` & rekening penyedia fallback Script Property. Taruna Rp0 dikecualikan; 1 AUDIT_LOG (NIT rekening terbaca, + `basis`/`sampai_tanggal` dicatat). Halaman TIDAK di-cache Dexie | ✅ diimplementasi |
...
| `ppk.kokpit` | PPK, KPA, WADIR3 (baca saja) | `{bulan}` → `{ringkasan:{bulan,target_rekap,terbayar_sp2d,outstanding_tagihan,porsi_dipesan,porsi_dimakan,nominal_pesanan_proyeksi,nominal_realisasi_sah,selisih_nominal,tanggal_kosong:[YYYY-MM-DD,...]}, tahapan:[{no,label,status:'hijau'\|'kuning'\|'merah'\|'n_a',angka,prasyarat_ok,link}] (9 langkah), tindakan:[{prioritas,apa,kenapa,link}]}` | **Kokpit PPK, murni agregasi READ-ONLY** — tidak menulis apa pun, tidak melewati guard action mana pun (`link` hanya menautkan ke halaman aksi; penegakan tetap di action masing-masing). Memanggil langsung `bayarList`/`spmList`/`sp2dRekonsiliasi`/`tagihanStatusDebet`/`tagihanList` yang sudah ada (bukan re-derive). **Degradasi anggun**: SPM kosong untuk bulan itu (legacy Jan-Mar 2026 atau belum ada PEMBAYARAN) → status `n_a`, BUKAN `merah`; SURAT_PENDEBETAN (§20, masih PARKIR) → langkah 8 selalu `n_a`. **`tanggal_kosong`** (Trigger Tahap 2 — deteksi dini, rumus sama persis `cetak.rekap_pesanan_bulan`): tanggal bulan itu s.d. HARI INI (bukan sisa bulan yang memang belum waktunya) yang sama sekali tidak punya baris PESANAN — muncul sebagai `tindakan` prioritas 1 begitu ada, menaut ke `/cetak/rekap-pesanan-bulan` untuk rincian; mencegah celah seperti Juli 2026 ketahuan mendadak sebelum acara (mis. wisuda) alih-alih dari hari pertama. **`nominal_pesanan_proyeksi`/`nominal_realisasi_sah`/`selisih_nominal`** (permintaan Firdaus — nilai rupiah pesanan vs realisasi langsung di kokpit): REUSE `_basisPesananBulan_` (14_rekap.gs) untuk proyeksi & SUM `REKAP_BULANAN.nominal` bulan itu untuk realisasi sah, pola identik `dashboard.running`. `tindakan` diurutkan tenggat-SP-terlewat dulu (prioritas 0). |
...
| Action | Role | Payload → Data | Keterangan |
|---|---|---|---|
| `penyedia.portal` | PENYEDIA | `{}` → `{penyedia:{nama,kontak,alamat,status}, kontrak:[{kontrak_id,harga_per_porsi,porsi_per_hari,harga_per_hari_efektif,tgl_mulai,tgl_akhir,status,menu:[{hari,menu_pagi,menu_siang,menu_malam}],lampiran:[{jenis,nama_file}]}], pesanan:[{tgl_makan,jml_taruna,menu,catatan,status}], realisasi:[{tanggal,porsi_diterima,jml_taruna_makan,ketidaksesuaian,tindak_lanjut}], pembayaran:[{bulan,nilai_total,gagal_debet,gagal_belum_lunas,berhasil_debet,gagal_sudah_diteruskan,tgl_diteruskan_terakhir,no_spm,tgl_spm,no_sp2d,tgl_sp2d,status,invoice_dikonfirmasi}], ringkasan_pembayaran:{dalam_proses,sudah_dibayar,total,total_berhasil_debet,total_gagal_debet}, rekap_berjalan:{bulan,jml_taruna,total_hari_makan,total_nominal,basis_pesanan:{bulan,hari_dipesan,oh_dipesan,nominal_proyeksi},status}|null}` | Semua data di-scope ke penyedia yang login (via `kontrak_id` miliknya). **`rekap_berjalan`** = agregat REKAP_BULANAN **bulan berjalan**, boleh dilihat penyedia **walau belum disetujui/FINAL** (dikonfirmasi Firdaus) — HANYA angka agregat (TANPA nit/nama/rekening/rincian per taruna, sesuai prinsip data sensitif portal); `status` disertakan agar frontend melabeli "Sementara — belum final" selama `!== 'FINAL'`; `null` bila bulan berjalan belum punya baris rekap. **Debet taruna→Senat** per bulan (dari sheet TAGIHAN gagal-debet, dicatat PPK via impor CSV; agregasi LEVEL BULAN karena TAGIHAN tak bertaut penyedia & model 1 katering utama/bulan): `gagal_debet` = SUM(TAGIHAN.nominal bulan itu), `gagal_belum_lunas` = yang status TERTAGIH/ESKALASI_MANUAL, `berhasil_debet` = `max(0, nilai_total − gagal_debet)` (basis nominal, belum dipotong biaya admin). **Kekurangan yang sudah ditagih ulang & diteruskan** ke penyedia: `gagal_sudah_diteruskan` = SUM(`nilai_transfer||nominal`) TAGIHAN status `LUNAS` yang punya `tgl_diteruskan_penyedia` bulan itu; `tgl_diteruskan_terakhir` = tanggal penerusan terakhir (`max`). Ditampilkan ringkas satu baris di portal ("Kekurangan sudah dilunasi & dikirim ke Anda"). READ-ONLY. `pesanan` HANYA status final `DISETUJUI`/`TERKIRIM`, `tgl_makan ≥` hari ini − 7. **SENGAJA TANPA** data per-taruna (nama/NIT), rekening, geotag realisasi, identitas staf internal (created_by/verif_by/approved_by/uploaded_by), dan **TANPA bantuan makan luar kampus** (BANTUAN_LUAR_KAMPUS/SP2D — transfer tunai ke taruna, bukan lewat kontrak penyedia). READ-ONLY (tanpa audit). |
