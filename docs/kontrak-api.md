...
| `kajur.hapus_absen` | KETUA_JURUSAN | `{nit, bulan}` → `{jml_dihapus, nit, bulan}` — **KOREKSI**: hapus semua baris STATUS_HARIAN berstatus ∈ STATUS_LUAR_KAMPUS satu taruna pada bulan itu (mis. absen salah input / salah bulan). BEDA dari `status.tandai_kembali` (yg hanya hapus hari KE DEPAN) — koreksi ini boleh menghapus tanggal LAMPAU. HANYA menyentuh status luar kampus (tak mengubah Pesiar/Cuti/Sakit). Scope prodi (`_pastikanTarunaProdi_`), withLock + audit (catat tanggal/status yg dihapus) |
