...
| `cetak.kuasa_debet_keluar` | **ADMIN, PPK, STAF_PPK** | `{bulan, nit_list[]}` → shape SAMA `cetak.form07` TAPI `pembayaran` bisa `null`. Dokumen **kuasa debet taruna KELUAR KAMPUS** (wisuda/magang/pindah) untuk SUBSET terpilih (nit_list wajib) — nominal bulan berjalan, `nilai_debet = nominal − biayaAdminBank`, per bank, tiap taruna TTD (kuasa debet). Reuse `_daftarKuasaDebet_(bulan, nit_list)` (dipakai bersama `cetak.form07`). **PEMBAYARAN OPSIONAL** (dokumen bisa disiapkan SEBELUM pembayaran dibuat — beda dari form07 yang mewajibkannya): bila ada → header SPM/kontrak terisi, bila belum → `pembayaran:null` & rekening penyedia fallback Script Property. Taruna Rp0 dikecualikan; 1 AUDIT_LOG (NIT rekening terbaca). Halaman TIDAK di-cache Dexie | ✅ diimplementasi |
